...
**Sentyment:** przy `sentiment.enabled: true` wektor per alt jest budowany z tabeli SQLite (`sentiment.news.sqlite_path`): wpisy `MICRO` mapują tickery na symbole CCXT (np. `SOL` → `SOL/USD:USD` na Kraken Futures), `MACRO` daje składową globalną (średnia z decay) i jest mieszana do każdego tokenu wg `macro_blend_into_tokens`; `NOISE` jest pomijane. Decay: `sentiment_score * (decay_base ** godzin_od_publikacji)`. Worker `tinyquant-h4 news-sync` / `news-loop` zapełnia bazę (RSS → Ollama JSON). Opcjonalnie `append_macro_to_regime_features` dokleja makro do wektora cech reżimu (zmiana wymiaru — ponowny trening XGBoost).
